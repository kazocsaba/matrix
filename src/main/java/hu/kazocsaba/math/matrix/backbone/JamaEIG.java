...
**/
class JamaEIG implements EigenDecomposition {
/* ------------------------
   Class variables
 * ------------------------ */
 ...
 * ------------------------ */
   ...
   private void orthes (double[][] V, double[][] H) {
   ...
   private void hqr2 (double[][] V, double[][] H) {
 ...
 * ------------------------ */
   ...
   */
   ...
   public JamaEIG (Matrix Arg) {
		if (Arg.getRowCount()!=Arg.getColumnCount()) throw new IllegalArgumentException("Eigenvalue decomposition only applies to square matrices");
 ...
 * ------------------------ */
   ...
   */
   ...
   public Matrix getD () {
      Matrix X = MatrixFactory.createMatrix(n,n);
      for (int i = 0; i < n; i++) {
         X.set(i,i,d[i]);
         if (e[i] > 0) {
				X.set(i,i+1,e[i]);
